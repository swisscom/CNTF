...
In dynamic, distributed environments, there are many parameters that need to be planned, defined, provisioned and managed across versions, generations, change-management integration, validation and reconciled upon the live state to desired state discrepancies.
...
- Moving away from direct imperative and sequential configuration in a day 0, 1, 2/n manner based on e.g. NETCONF, CLI, SNMP to a declarative source-of-truth mastered in GitOps, synced and reconciled e.g. with K8s operators.
...
- Instead of managing a flat model of well over 1000 parameters per NF, abstract configuration artifacts e.g. CRDs should be provided that reflect repetitive use cases of the business. Examples from the 5G Core Network Platform of such repetitive configuration tasks are:
  ...
  5. Routing behaviour for P-GW/UPF contexts used by Private Networks.
  6. Installation of a new data plan (PCF)
  7. Management of Secrets & Certificates (E2E)
  8. Management of telemetry profiles (destination, datapoints, periodicity etc)
  9. IP/FQDN Claims for day 0 instantiation
  10. Primary & Secondary Network configuration incl. L2 provisioning
  11. Deployment of VMs or K8s Cluster as a Service and Namespace as a Service
  12. Management of roaming partners.
...
| AREQ A2-1.005 | Provide software enabling CSPs to manage the lifecycle of the K8s clusters, handle updates, and ensure that the desired state is maintained |

## 2 - 6 CNTF’s ask to CSP readiness

| Requirement   | Description                                                  |
| ------------- | ------------------------------------------------------------ |
| AREQ A2-2.001 | Git-based process for configuration management i.e. all configuration is applied via Git and NF-local configuration procedure (i.e. CLI) are disabled by default and only used in emergencies |
